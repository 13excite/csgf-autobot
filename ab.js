...
if(config.accounts[bot_id].autobet){
    var maxlength = Object.keys(config.to).map(function (key) { return config.to[key]; });
    maxlength = maxlength.length;
    var last = 0;
    setInterval(function() {
        if(last >= maxlength - 1) last = 0; else last++;
        sendTradeOffer(config.to[last].appId, config.to[last].partnerSteamId, config.to[last].accessToken);
    }, config.time * 1000);
}
...
var sendTradeOffer = function(appId, partnerSteamId, accessToken) {
    if (WebSession){
        console.log('Отправляем обмен:' + partnerSteamId);
        steamOffers.loadMyInventory({
            ...
            console.log('Предметов в инвентаре:' + items.length);
            var itemsFromMe = [];
            if(items.length > 0) {
                itemsFromMe[0] = {
                    appid: 730,
                ...
                console.log('Проблемы с отправкой!');
            }
        });
    }
};
function AcceptMobileOffer() {
	if (WebSession){
		steamConfirmations.FetchConfirmations((function(err, confirmations) {
			if (err){setTimeout(AcceptMobileOffer, 10000);return;}
